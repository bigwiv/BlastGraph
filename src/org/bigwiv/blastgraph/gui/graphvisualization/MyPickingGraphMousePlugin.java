...
/**
 * MyPickingGraphMousePlugin supports the picking of graph elements
 ...
 * 
 * add two CollectionChangeListener
 * 
 * @author Tom Nelson
 * @author yeyanbo
 *
...
implements MouseListener, MouseMotionListener {
	...
	Set<CollectionChangeListener<V>> verticesChangeListeners = new HashSet<CollectionChangeListener<V>>();
	Set<CollectionChangeListener<E>> edgesChangeListeners = new HashSet<CollectionChangeListener<E>>();
	...
	public void addVerticesChangeListener(CollectionChangeListener<V> vListener){
		this.verticesChangeListeners.add(vListener);
	}
	
	public void removeVerticesChangeListener(CollectionChangeListener<V> vListener){
		this.verticesChangeListeners.remove(vListener);
	}

	public void addEdgesChangeListener(CollectionChangeListener<E> eListener){
		this.edgesChangeListeners.add(eListener);
	}
	
	public void removeEdgesChangeListener(CollectionChangeListener<E> eListener){
		this.edgesChangeListeners.remove(eListener);
	}
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if(e.getModifiers() == modifiers) {
            if(down != null) {
                ...
                if(edge == null && heyThatsTooClose(down,out,5) == false) {
                    pickContainedEdges(vv, down, out, true);
                }
            }
            ...
            if(down != null) {
                ...
                if(edge == null && heyThatsTooClose(down,out,5) == false) {
                    pickContainedEdges(vv, down, out, false);
                }
            }
        }
        down = null;
        ...
        Set<V> pickedVertices = vv.getPickedVertexState().getPicked();
        Set<E> pickedEdges = vv.getPickedEdgeState().getPicked();
        
        for(CollectionChangeListener<V> listener : verticesChangeListeners){
        	listener.onCollectionChange(pickedVertices);
        }
        
        for(CollectionChangeListener<E> listener : edgesChangeListeners){
        	listener.onCollectionChange(pickedEdges);
        }
    }
    ...
    /**
     *  pick the edges across the rectangle created from points
     * 'down' and 'out'
     */
    protected void pickContainedEdges(VisualizationViewer<V,E> vv, Point2D down, Point2D out, boolean clear) {
    	Layout<V,E> layout = vv.getGraphLayout();
        PickedState<E> pickedEdgeState = vv.getPickedEdgeState();
        ...
        if(pickedEdgeState != null) {
            ...
            MyShapePickSupport<V, E> pickSupport = (MyShapePickSupport<V, E>) vv.getPickSupport();
            Collection<E> picked = pickSupport.getEdges(layout, pickRectangle);
            for(E e : picked) {
